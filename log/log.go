...
func Init(level string, outputs ...string) error {
	if zapLog, _, err := newLogger(level, outputs...); err != nil {
		return err
	} else {
		log = &Logger{x: zapLog}
	}
...
func newLogger(logLevel string, outputs ...string) (*zap.SugaredLogger, *zap.AtomicLevel, error) {
...
func getDefaultLog() *Logger {
	if log == nil {
		panic("initialize log before use")
	}
	return log
}
